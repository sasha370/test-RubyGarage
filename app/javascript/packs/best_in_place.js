...
BestInPlaceEditor.prototype = {
    ...
    activate: function () {
        ...
        if (this.isPlaceHolder()) {
            ...
            switch (this.formType) {
                ...
                case 'datetime':
                    to_display = this.element.toLocaleString();
    ...
    update: function () {
        ...
        if (this.formType in {"input": 1, "textarea": 1, "date": 1, "datetime": 1} && value === this.oldValue) {
            this.abort();
            return true;
        }
    ...
    "datetime": {
        activateForm: function () {
            ...
            var input_elt = jQuery(document.createElement('input'))
                ...
                .attr('name', this.attributeName)
                .val(new Date(this.display_value).toISOString().split('T')[0]);
            // alert()
            // Add class to form input
            if (this.inner_class) {
                input_elt.addClass(this.inner_class);
            ...
            this.element.find("input[type='datetime-local']")[0].select();
            this.element.find("form").bind('submit', {editor: this}, BestInPlaceEditor.forms.input.submitHandler);
            if (this.cancelButton) {
                this.element.find("input[type='button']").bind('click', {editor: this}, BestInPlaceEditor.forms.input.cancelButtonHandler);
            }
            if (!this.okButton) {
                this.element.find("input[type='datetime-local']").bind('blur', {editor: this}, BestInPlaceEditor.forms.input.inputBlurHandler);
            }
            this.element.find("input[type='datetime-local']").bind('keyup', {editor: this}, BestInPlaceEditor.forms.input.keyupHandler);
            this.blurTimer = null;
            this.userClicked = false;
